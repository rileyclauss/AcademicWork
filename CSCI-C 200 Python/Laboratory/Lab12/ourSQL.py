...
def createTable(cursor, tableName, columns):
    ...
    theQuery = "CREATE TABLE {0} ({1})".format(tableName, columns)
    cursor.execute(theQuery)
...
def insertValues(cursor, tableName, valueString):
    ...
    """
    # TODO
    theQuery = "INSERT INTO {0} VALUES ({1})".format(tableName, valueString)
    cursor.execute(theQuery)
...
def queryStatement(theSelect, theFrom, theWhere=""):
    ...
    # TODO
    if theWhere == "":
        theQuery = "SELECT {0} FROM {1}".format(theSelect, theFrom)
    else:
        theQuery = "SELECT {0} FROM {1} WHERE {2}".format(theSelect, theFrom, theWhere)
    return theQuery
...
if __name__ == "__main__":
    ...
    query = queryStatement("Owner.Name, Animal.Name", 
                            "Owner, Animal", 
                            "Animal.Type == 'Cat' and Animal.ID == Owner.AnimalID") # TODO: Fill this in
    ...
    query = queryStatement("Owner.Name, Animal.Name", "Owner, Animal", "Animal.ID == Owner.AnimalID and Animal.Age <= 3") # TODO: Fill this in
    for x in theCursor.execute(query):
        print(x)
    ...
    query = queryStatement("Count(distinct Vet.VetID)", "Vet, Owner o1, Owner o2", "o1.name != o2.name and Vet.VetID == o1.VetID and Vet.VetID == o2.VetID") # TODO: Fill this in
    for x in theCursor.execute(query):
        print(x)
